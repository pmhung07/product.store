...
use App\Http\Requests\WarehousePhRequest;
use App\Http\Requests\WarehousePhDetailsRequest;

use App\WarehousePh;
use App\Warehouse;
use App\Supplier;
use App\WarehousePhDetails;
use App\Product;
use App\WarehouseInventory;

class WarehousePhController extends Controller
{
    public function getIndex(Request $request){
    	...
    	$sort='created_at';
		$order='desc';
		$rows = WarehousePh::select('warehouse_ph.*','users.id as user_id','users.name as user_name','users_position.name as users_position_name')
                                ->join('users', 'users.id', '=', 'warehouse_ph.admin_id')
                                ->leftjoin('users_position', 'users_position.id', '=', 'users.user_position_id')
								->join('warehouse', 'warehouse.id', '=', 'warehouse_ph.warehouse_id')
								->join('supplier', 'supplier.id', '=', 'warehouse_ph.supplier_id');

		if ($request->has('wareph_name')){
			$rows = $rows->where('warehouse_ph.name', 'LIKE', '%'.$request->input("wareph_name").'%');
		}
		if ($request->has('wareph_code')){
			$rows = $rows->where('warehouse_ph.code', 'LIKE', '%'.$request->input("wareph_code").'%');
		}
